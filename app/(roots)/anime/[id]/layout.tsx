    ...
    <main>
      ...
      <div className="flex flex-col items-start px-4 sm:px-6 w-full">
        <div className="w-full max-w-[2000px] lg:mx-10 flex flex-col md:flex-row gap-6 md:gap-8">
          <div
            className={`w-full md:w-[230px] ${anime.bannerImage ? "mt-4 md:mt-8 " : "mt-4"} shrink-0 flex flex-col md:items-start`}
          >
            <div className="flex max-[600px]:flex-col gap-3">
              <div className="max-[600px]:flex justify-between ">
                <div
                  className={`${anime.bannerImage ? `max-[600px]:relative max-[600px]:z-10 max-[600px]:-mt-22` : `max-[600px]:block`}`}
                >
                  <Image
            ...
            <div className="hidden md:flex flex-col justify-center my-4 w-[230px] ">
              ...
              <div className="">
                {anime.startDate && (
                  <div className="flex flex-col mt-4 px-2">
                    ...
                    <span className="text-(--color-muted) font-semibold">
                      {" "}
                      {getMonthName(anime.startDate.month)}{" "}
              ...
              <div className="">
                {anime.producers?.nodes &&
                  anime.producers.nodes.length > 0 && (
                    <div className="flex flex-col mt-4 px-2">
          ...
          <div className={`flex-1 w-full flex flex-col gap-4 min-w-0`}>
            <div
              className={`flex flex-col min-h-16 md:min-h-82 justify-center gap-2 ${anime.bannerImage ? `mt-4 md:mt-5` : `mt-4`} `}
            >
                      ...
                      {streamingLinks.map((link) => {
                        ...
                        return (
                          <Link
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg text-foreground sm:rounded-xl text-xs sm:text-sm font-medium hover:opacity-90 transition-opacity"
                            style={{ background: meta.brandColor }}
                          >
                            {meta.icon && (
                              <img
              ...
              <div className="text-(--color-muted) text-sm sm:text-base leading-relaxed md:pr-10" dangerouslySetInnerHTML={{ __html: anime.description ?? "" }} />
            </div>

            {/* Navigation Tabs */}
            <AnimeNavigationTabs navigationItems={navigationItems} animeId={anime.id} />
